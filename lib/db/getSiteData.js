...
export async function getGlobalData({
  ...
  const safeData = handleDataBeforeReturn(
    deepClone(data || EmptyData(pageId)),
    locale
  )
  if (!safeData?.siteInfo) {
    console.error('[getGlobalData] Missing siteInfo', {
      pageId,
      from,
      keys: safeData ? Object.keys(safeData) : []
    })
  }
  return safeData
}
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  if (!data || Object.keys(data).length === 0) {
    console.error('[getSiteDataByPageId] Empty response, falling back to placeholder', pageId)
...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  const allPages = collectionData.filter(post => {
    if (post?.type === 'Post' && post.status === 'Published') {
      postCount++
      if (post?.publishDate) {
        if (!firstPostDate || post.publishDate < firstPostDate) {
          firstPostDate = post.publishDate
        }
        if (!latestPostDate || post.publishDate > latestPostDate) {
          latestPostDate = post.publishDate
        }
      }
      if (post?.lastEditedDate) {
        const editTime = new Date(post.lastEditedDate).getTime()
        if (!latestUpdatedDate || editTime > latestUpdatedDate) {
          latestUpdatedDate = editTime
        }
      }
    }
  ...
  return {
    ...
    siteStats: {
      postCount,
      firstPostDate: firstPostDate ? new Date(firstPostDate).toISOString() : null,
      latestPostDate: latestPostDate ? new Date(latestPostDate).toISOString() : null,
      latestUpdatedDate: latestUpdatedDate
        ? new Date(latestUpdatedDate).toISOString()
        : null
    },
    pageIds,
    latestPosts
 ...
 * @param {*} db
 */
function handleDataBeforeReturn(db, locale) {
  // 清理多余数据
  delete db.block
  delete db.schema
  ...
  applyLocaleHref(db, locale)

  return db
}

function applyLocaleHref(db, locale) {
  const prefix = normalizeLocalePrefix(locale)
  if (!prefix) {
    return
  }

  const addPrefix = href => {
  ...
  const patchItem = item => {
    ...
    if (typeof item.href === 'string') {
      item.href = addPrefix(item.href)
    }
    if (Array.isArray(item.children)) {
      item.children.forEach(child => patchItem(child))
    }
  }

  const collections = ['allPages', 'latestPosts', 'allNavPages', 'customMenu']
  collections.forEach(key => {
    const collection = db?.[key]
    if (Array.isArray(collection)) {
      collection.forEach(item => patchItem(item))
    }
  })
