...
export function GlobalContextProvider(props) {
  ...
  // 登录验证相关 - 使用增强的 Clerk 认证 hook
  const enableClerk = process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY

  const showClerkAlert = message => {
  ...
  const clerkSetupMessage =
    '登录功能需要配置 Clerk 认证服务。\n\n请在 .env.local 文件中添加:\nNEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=your_key\nCLERK_SECRET_KEY=your_secret\n\n详情请访问: https://clerk.com/'

  const clerkAuth = enableClerk
    ...
    : {
        ...
        userPreferences: {},
        signOut: () => {},
        openSignIn: () => showClerkAlert(clerkSetupMessage),
        openSignUp: () =>
          showClerkAlert(
            '注册功能需要配置 Clerk 认证服务。\n\n请在 .env.local 文件中添加:\nNEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=your_key\nCLERK_SECRET_KEY=your_secret\n\n详情请访问: https://clerk.com/'
          ),
        openUserProfile: () =>
          showClerkAlert('用户中心需要配置 Clerk 认证服务。'),
        hasPermission: () => false
      }
  ...
  return (
    <GlobalContext.Provider
      value={{
        ...
        postCount: postCount || siteStats?.postCount || 0,
