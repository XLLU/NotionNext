...
export function adjustPageProperties(properties, NOTION_CONFIG) {
  ...
  if (isHttpLink(properties?.href)) {
    // 开发环境下的特殊处理：如果是生产环境的语言切换链接，转换为本地链接
    if (process.env.NODE_ENV === 'development') {
      const prodUrl = properties?.slug
      // 检查是否为生产环境的语言切换链接
      if (prodUrl?.includes('freemium.cc')) {
        if (prodUrl.includes('/en')) {
          // 英文链接转换为本地英文
          properties.href = '/en'
          properties.target = '_self'
        } else if (prodUrl === 'https://www.freemium.cc/' || prodUrl === 'https://www.freemium.cc') {
          // 中文链接转换为本地中文
          properties.href = '/'
          properties.target = '_self'
        } else {
